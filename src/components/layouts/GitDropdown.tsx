    ...
    return (
        <div ref={buttonRef}>
            ...
            >
                ...
                <span className="text-sm hidden md:inline">Commit</span>
            </Button>

            {isOpen && mounted && createPortal(
                <div
                    ref={dropdownRef}
                    className="fixed bg-gray-200 rounded-xl shadow-xl shadow-gray-300/70 border border-gray-300 py-2 z-[9999] origin-top-right animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-300 transition-all w-80"
                    style={{
                ...
                >
                    ...
                    <div className="px-3 pb-2 pt-1 animate-in slide-in-from-top-2 fade-in duration-200">
                        ...
                        <textarea
                            className="w-full text-sm p-2 rounded-md border border-neutral-300 focus:border-indigo-500 focus:ring-0.5 focus:ring-indigo-500 outline-none resize-none bg-white text-neutral-700 disabled:opacity-50"
                            rows={3}
                        ...
                        <input
                            type="text"
                            className="w-full text-sm px-2 py-1.5 rounded-md border border-neutral-300 focus:border-indigo-500 focus:ring-0.5 focus:ring-indigo-500 outline-none bg-white mb-1"
                            placeholder="main"
                            value={branchName}
                            onChange={(e) => setBranchName(e.target.value)}
                        />
                        <div className="flex justify-end gap-2 mt-2">
                            <Button size="sm" variant="ghost" onClick={() => { setIsOpen(false); setShowCommitInput(false); }} className="h-7 text-xs px-2" disabled={isLoading}>
                                Cancel
                            </Button>
